...
public class Client {
	...
	public static void main(String[] args) {
		...
		BufferedReader br; //gets from server

		
		try{
			clientSock = new Socket("localhost", portNo);
			ps = new PrintWriter(clientSock.getOutputStream(),true);
			ir = new InputStreamReader(clientSock.getInputStream());
			br = new BufferedReader(ir); //gets from server
				...
				if(menuNo == 1 || menuNo == 2 || menuNo == 3){
					...
					if(menuNo == 1){
						...
						while(validUN == 0){ // ask for the username till it is valid
							...
							validUN = Integer.parseInt(br.readLine());
							...
							if(validUN == 1){ // ask for the other attributes after the username is valid
								...
								cont = password + "==="+ email + "===" + name + "===" + surname + "===" + type;
								ps.println(cont);
							}
							else{
						...
						while(validUN == 0){ // ask for the username till it is valid
							...
							if(validUN == 1){ // ask for the other attributes after the username is valid
								...
								while(validPass == 0){ //ask for the password till it is valid
									password = scan.nextLine();
									ps.println(password);
									validPass = Integer.parseInt(br.readLine());
												...
												if(noR == 1 || noR == 2 || noR == 3 || noR == 4){
													...
													if(noR == 1){ // send message
														...
														while(validEmailTo == 0){
															...
															ps.println(emailTo);
															validEmailTo = Integer.parseInt(br.readLine());
															if(validEmailTo == 1){
																String emailMsg;
														...
														if(buf == null || buf.equals(""))
															...
															System.out.println("|From id\t|Message\t\t\t|Time and Date");
														...
														if(buf == null || buf.equals(""))
															...
															StringTokenizer st = new StringTokenizer(buf, "???");
															System.out.println("|To id\t|Message\t\t\t|Time and Date");
															while(st.hasMoreTokens()) System.out.println(st.nextToken());
														}
													}
												}
												...
												if(noA == 1 || noA == 2 || noA == 3 || noA == 4 || noA == 5 ){
													...
													if(noA == 1){ // create
														...
														while(validUNCreate == 0){ // ask for the username till it is valid
															...
															validUNCreate = Integer.parseInt(br.readLine());
															if(validUNCreate == 1){ // ask for the other attributes after the username is valid
																...
																cont = passwordCreate + "==="+ emailCreate + "===" + nameCreate + "===" + surnameCreate + "===" + typeCreate;
																ps.println(cont);
															}
															else{
																System.out.println("The username is not valid, please try again!");
														...
														while(validUNRead == 0){ // ask for the username till it is valid
															...
															validUNRead = Integer.parseInt(br.readLine());
															if(validUNRead == 1){ 
														...
														while(validUNUpdate == 0){ // ask for the username till it is valid
															...
															validUNUpdate = Integer.parseInt(br.readLine());
															if(validUNUpdate == 1){ 
														...
														while(validUNDelete == 0){ // ask for the username till it is valid
															...
															validUNDelete = Integer.parseInt(br.readLine());
															if(validUNDelete == 1) 
